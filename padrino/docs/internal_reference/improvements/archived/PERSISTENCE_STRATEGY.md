...
Este documento define a separação de responsabilidades entre o **Conector de Banco de Dados** (Infraestrutura) e a *
*Camada de Repository** (Adapters), seguindo os princípios da Arquitetura Hexagonal adotada no projeto **Don**.
...
* **Unit of Work:** Os Casos de Uso devem coordenar transações através de interfaces, sem conhecer detalhes de
  implementação do driver SQL.
* **Independência de Driver:** O Repository deve ocultar o driver específico (ex: PostgreSQL, MySQL) para que o domínio
  permaneça puro.
